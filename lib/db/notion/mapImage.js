...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // Private Content Library assets cannot use Notion's public image proxy.
  // Keep the database private and let our server validate the owning page
  // against the production gate before returning image bytes.
  const isPrivateNotionAsset = isPrivateNotionAssetUrl(ret)
  if (BLOG.CONTENT_SOURCE !== 'legacy' && block?.id && isPrivateNotionAsset) {
    const target = block?.type === 'page' ? 'page' : 'block'
    return `/api/notion-image?id=${encodeURIComponent(block.id)}&target=${target}`
  }
  ...
  const needConvert =
    (!hasConverted && (block.type === 'bookmark' || isPrivateNotionAsset)) ||
...
function isPrivateNotionAssetUrl(value) {
  try {
