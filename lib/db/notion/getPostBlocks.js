...
export async function fetchNotionPageBlocks(
  ...
  const pageBlock = await getOrSetDataWithCache(cacheKey, () =>
    limit(() =>
      shouldUseOfficialContentLibrary()
        ? fetchOfficialPageRecordMap(id)
        : getPageWithRetry(id, from, 3, cacheKey)
    )
  )
  ...
  if (shouldUseOfficialContentLibrary()) {
    removeInternalPublishingWorkspaces(pageBlock)
  }
