...
export function adaptContentLibraryProperties(properties = {}) {
  ...
  properties.tags = []
  properties.slug = properties[CONTENT_LIBRARY_FIELDS.contentId] || properties.id
...
export function isWebsiteEligibleContent(properties = {}) {
  ...
  if (properties.status !== 'Published') return false

  const previewAllPublished =
    process.env.VERCEL_ENV === 'preview' &&
    process.env.CONTENT_LIBRARY_PREVIEW_ALL_PUBLISHED !== 'false'

  return previewAllPublished || properties.publishChannels?.includes('网站')
}
