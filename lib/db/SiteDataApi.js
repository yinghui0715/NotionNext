...
import {
  getContentLibrarySiteConfig,
  isContentLibrarySchema,
  isWebsiteEligibleContent
} from './notion/contentLibraryAdapter'
import {
  fetchOfficialContentLibrarySiteData,
  shouldUseOfficialContentLibrary
} from './notion/officialContentLibrary'
import { preserveLastSuccessfulSnapshot } from './notion/contentSourcePolicy'
import {
...
export async function fetchGlobalAllData({
  ...
  const cachedData = await getOrSetDataWithCache(cacheKey, async () => {
    ...
    try {
      ...
      // Official Content Library is the production source of truth. Let ISR
      // keep serving the last successful page when Notion has a transient
      // failure instead of caching EmptyData and replacing valid routes with
      // a temporary 404 for the cache lifetime.
      preserveLastSuccessfulSnapshot(error, shouldUseOfficialContentLibrary())
    }
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const data = await getOrSetDataWithCache(cacheKey, async () => {
    ...
    if (shouldUseOfficialContentLibrary()) {
      return fetchOfficialContentLibrarySiteData(pageId)
    }
    // 拉取数据
...
function cleanPostForClient(post) {
  ...
  const rootEntry = post.blockMap.block?.[rootId]
  const rootContent = Array.isArray(post.content) ? post.content : null
  const cleanedPost = cleanBlock(post)
  delete cleanedPost.content
  // react-notion-x treats the first block in the record map as the page root.
  // Preserve and explicitly lead with the synthetic root produced by the
  // official Notion API adapter; otherwise the first callout/text block can be
  // mistaken for the root and the remaining article body is not rendered.
  if (rootEntry && cleanedPost.blockMap?.block) {
    const cleanedRoot =
      cleanedPost.blockMap.block[rootId] || cleanBlock(rootEntry)
    if (rootContent) cleanedRoot.value.content = rootContent
    const children = { ...cleanedPost.blockMap.block }
    delete children[rootId]
    cleanedPost.blockMap.block = { [rootId]: cleanedRoot, ...children }
  }
...
async function convertNotionToSiteData(
  ...
  const contentLibraryMode = isContentLibrarySchema(schema)
  const NOTION_CONFIG = (await getConfigMapFromConfigPage(collectionData)) || {}
  if (contentLibraryMode) {
    Object.assign(NOTION_CONFIG, getContentLibrarySiteConfig())
  }
  if (process.env.NODE_ENV === 'development') {
    console.log(
...
function getCategoryOptions(schema) {
  ...
  const categorySchema = Object.values(schema).find(
    e => e.name === BLOG.NOTION_PROPERTY_NAME.category || e.name === 'Category'
  )
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || 'Leo 数字工坊'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION ||
    'Leo数字工坊记录 AI Agents、工作流自动化、Engineering AI 与数字系统实践，分享从真实问题到可复用解决方案的构建过程。'
  ...
  // An explicitly enabled TITLE in Notion's config table is a deliberate
  // site-brand override. Keep the database name as the fallback so content
  // owners can preserve an internal/archive database name independently.
